...
function Modal({ children, title, onCancel }: PropsWithChildren<Props>) {
	return (
		<div className={style.wrapper} role="dialog">
			...
			<div className="relative max-w-sm rounded-lg bg-white px-4 pb-4 transition-all duration-300 dark:bg-navy-700 sm:px-5">
				<div className="my-3 flex h-8 items-center justify-between">
					...
					<button
						onClick={onCancel}
						className="hidden btn -mr-1.5 h-7 w-7 rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25"
					>
						<svg
				...
				<div className="mt-4 flex w-full gap-4">{children}</div>
			</div>
		</div>
	);
}
